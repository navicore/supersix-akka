package onextent.supersix.akka.models.functions

import java.time.{ZoneOffset, ZonedDateTime}
import java.util.{Date, UUID}

import akka.http.scaladsl.marshallers.sprayjson.SprayJsonSupport
import onextent.supersix.akka.models._
import spray.json._
...
trait JsonSupport extends SprayJsonSupport with DefaultJsonProtocol {
  ...
  implicit val masterFormat: RootJsonFormat[Master] = jsonFormat5(
    Master)

  implicit val loadMasterReqFormat: RootJsonFormat[LoadMasterRequest] =
    jsonFormat2(LoadMasterRequest)
